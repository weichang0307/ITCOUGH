...
class FragmentWeek : Fragment() {
    ...
    private var startDate: LocalDate = LocalDate.now().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY))
    ...
    private fun getAnalysisData() : Map<String, Int>? {
        val jsonData = Gson().toJson(mapOf(
            ...
            "endDate" to startDate.plusWeeks(1).toString() //yyyy-mm-dd
        ))
        Connection.sendJsonPostRequest(
            ...
            onRequestSuccess = { con ->
                ...
                requireActivity().runOnUiThread {
                    ...
                    tvDuration.text = "${startDate.toString().replace('-','/')} - ${startDate.plusDays(6).toString().replace('-','/')}"
                    tvAmount.text = jsonArray.length().toString()
                    scatterChartView.updateData(data, ScatterChartView.WEEK)
                }
